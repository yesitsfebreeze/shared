...
- [ ] `conserved/tests/load_scope.rs` exists and its **entire** contents are
      ...
      non-zero `N passed`.
- [ ] `panic_unwinds_all_n_in_reverse` — registers `N = 100_000` effects on one
...
- [ ] That test's failure output does not dump 100_000 elements: on mismatch
...
- [ ] `panic_unwinds_all_n_in_reverse` never calls `Scope::close` — the unwind
      ...
      one complements.
- [ ] `panic_unwind_is_not_quadratic` — times the unwind of `N / 8 = 12_500`
...
- [ ] `deep_nesting_unwinds_at_a_safe_depth` — 1_000 nested scopes, each held
...
- [ ] `held_reports_all_n_before_the_panic` — after registering `N` effects,
      ...
      covers.
- [ ] `disposed_effects_are_not_replayed_by_the_panic` — registers `N`
...
- [ ] **Every one of these tests can fail.** For at least
...
- [ ] No `#[ignore]`, no `cfg(feature = ...)`, no `N` that changes with the
      profile. One `N`, one value, always run.
- [ ] `conserved/Cargo.toml` is byte-identical after this spec — no new
      dev-dependency; `std::time::Instant` and `std::panic::catch_unwind` are
      the whole harness.
- [ ] Total added wall time for this file is under 3 s in dev on the
