...
- [ ] `git diff --stat conserved/Cargo.toml` shows no change from this spec, and
      `cargo tree -p conserved --edges normal --depth 1` with no features is
      unchanged (`blake3` only, no `serde`).
- [ ] p2's amended manifest gate test still passes unmodified. If this spec had
      to edit it, the ordering precondition above was violated.
- [ ] `cargo build -p conserved` with **no** features compiles, and `Instant`
      has no `Serialize`/`Deserialize` impl in that build (the `#[cfg]` is on the
      impls, not on a stub).
- [ ] `clock_serde_binary_encoding_is_identical_to_i64` — for
...
- [ ] `clock_serde_struct_substitution_is_wire_compatible` — postcard-encodes
      ...
      there.
- [ ] `clock_serde_postcard_round_trips` —
      `postcard::from_bytes::<Instant>(&postcard::to_stdvec(&i)?)? == i` for all
      five values above.
- [ ] `clock_serde_json_is_a_number_not_a_string` —
...
- [ ] `clock_serde_json_round_trips_including_negative_and_extremes` —
...
- [ ] `clock_serde_json_rejects_non_integers` —
...
- [ ] `clock_serde_deserialize_uses_from_unix_nanos` — deserializing `0` yields
...
- [ ] `FixedClock` and `SystemClock` gain **no** serde impls. A clock is not
      data; only the instant it produced is. The doc comment states the refusal.
- [ ] The impls' doc comment states in one sentence: the encoding is the i64
...
- [ ] A note in the same doc comment records the one place transparency does
...
- [ ] spec02's `clock_system_clock_is_the_only_reader` and p2's
