...
| `- [x]` | a closure |
| the decision | `prds/memos/done-counts-which-boxes.md`, taken by the user 2026-08-27 |
...
|---|---|---|
| mitosys | `prds/p6-rust-core/p6k-kern-merge/p6k9-fold-and-seq` | 1 |
| mitosys | `prds/p6-rust-core/p6k-kern-merge/p6k8-read-path-remaining` | 3 |
| mitosys | `prds/p7-plugin-abi/p7e-wasm-host` | 6 |
| mitosys | `prds/p7-plugin-abi/p7f-arena` | 4 |
| mitosys | `prds/p7-plugin-abi/p7b-service-seam` | 4 |
| **mitosys** | **5 PRDs** | **18** |
| shared | `prds/p5-adoption/ratchets` | 4 |
| shared | `prds/p5-adoption/realm` | 3 |
| **shared** | **2 PRDs** | **7** |
| **all** | **7 PRDs** | **25** |
...
unchanged: the master board's own `prds/shared-crate-home` remains the one
`done` PRD no gate can reach.

## Nothing covers the master board

`prds/` — the master board at `/Users/feb/dev/infra/prds` — has **no gate, and
...
**`prds/shared-crate-home` is `state: done` with 5 unticked `## Acceptance`
...
That file belongs to **`pearde/prds`**, by that board's own admission rule:
...
is `pearde/prds/finished-counts-both-files`, and no tree's board edits it.

## What each tree owes

| tree | child PRD | owed |
|---|---|---|
| mitosys | `prds/exemptions-name-their-reason` | widen to whole-file; replace one reason string with 8 entries that each name a PRD, a commit and a distinct removal condition |
| shared | `prds/exemptions-name-their-reason` | widen to whole-file; resolve `p5-adoption/ratchets` and `p5-adoption/realm` by one of the three forms; `EXEMPT` stays `&[]` |
| model | `prds/exemptions-name-their-reason` | port `realm/src/gates/tests/done_boxes_are_ticked.rs`; land `EXEMPT` empty; close `next-wave/sampler` |
| realm | `prds/exemptions-name-their-reason` | delete the one dead `EXEMPT` entry; no scope change owed |
