...
- [ ] Before moving anything, record the two baselines and paste them into your
...
- [ ] The move is done and nothing was lost: `git ls-files shared/ | wc -l`
...
- [ ] `Cargo.toml` line 3 reads `members = ["shared"]` and its `serde` comment
...
- [ ] `git grep -nI conserved -- shared/` returns **exactly 5** lines, and they
      are the five listed above — counted with `| wc -l`, not eyeballed
- [ ] `Cargo.lock` was regenerated by cargo, not edited: `git diff -- Cargo.lock`
      shows only the package-name/ordering change, and
      `git grep -nI conserved -- Cargo.lock` returns nothing
- [ ] **Parity, not an absolute.** `cargo test --workspace --no-fail-fast`
...
- [ ] `cargo test -p shared -- --skip every_done_prd_has_no_unticked_box` exits
...
- [ ] `cargo fmt --check --all` clean, and the files `cargo fmt --all` changed
...
# the census that must come out at exactly 5
git grep -nI conserved -- shared/
git grep -cI conserved -- shared/ | awk -F: '{s+=$2} END {print "in-crate survivors:", s}'

# manifests
git grep -nI 'members\|^name = ' -- Cargo.toml shared/Cargo.toml
git grep -nI conserved -- Cargo.toml Cargo.lock shared/Cargo.toml

# main must load a workspace — run against the COMMIT, not the dirty worktree
W=$(mktemp -d)/wt
git worktree add --detach "$W" HEAD >/dev/null
( cd "$W" && cargo metadata --no-deps --format-version 1 >/dev/null; echo "metadata exit=$?" )
