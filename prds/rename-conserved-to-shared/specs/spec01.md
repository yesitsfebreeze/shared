...
- [x] `git grep --untracked -nI conserved -- shared/` returns **exactly 7** lines:
      the five listed above plus `shared/tests/landed_rev_is_published.rs:13`
      and `:29`, both the address `prds/rename-conserved-to-shared/prd.md`,
      immutable under rule 3 — counted with `| wc -l`, not eyeballed. Amended
      from 5 at collect: plain `git grep` skips the untracked gate file spec03 adds
